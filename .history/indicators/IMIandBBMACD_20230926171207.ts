# A SIMPLE indicator written by @diveyez which includes intraday momentum index and a custom MACD
declare lower;
...
def imi = if applySmoothing then EhlersSuperSmootherFilter(100 * (upt / (upt + dnt)), lowBand) else 100 * (upt / (upt + dnt));

plot imi_essf = filt;
plot imi_smooth = if applySmoothing then EhlersSuperSmootherFilter(100 * (upt / (upt + dnt)), lowBand) else Double.NaN;
imi_smooth.SetDefaultColor(Color.GREEN);
imi_smooth.SetLineWeight(2);

def basisx = ExpAverage(imi, lengthBB);
def devx = (mult * StDev(imi, lengthBB));
def ulx = (basisx + devx);
def llx = (basisx - devx);
...
# Shift MACD plot up for visibility
def MACD_Shifted = MACD + (2 * SignalLine);

# Plot MACD and Signal Line
plot MACDPlot = if IsNaN(close) then Double.NaN else MACD_Shifted;
MACDPlot.SetDefaultColor(Color.CYAN);
MACDPlot.SetLineWeight(2);

plot SignalLinePlot = if IsNaN(close) then Double.NaN else SignalLine + (2 * SignalLine);
SignalLinePlot.SetDefaultColor(Color.BLUE);
...
# Add legend
#AddLabel(yes, Concat("MACD(", fastLength, ", ", slowLength, ", ", MACDLength, ")"), Color.CYAN);
#AddLabel(yes, Concat("Signal(", MACDLength, ")"), Color.BLUE);
#AddLabel(yes, "IMI", Color.GREEN);
#AddLabel(yes, "Volatility Bands", Color.YELLOW);

# Add text to indicate overbought and oversold levels
#AddText(Concat("Overbought (", over_bought, ")"), Color.DARK_RED);
#AddText(Concat("Oversold (", over_sold, ")"), Color.DARK_GREEN);

# Hide price chart and volume subgraph
#hideprice();
#hidevolume();